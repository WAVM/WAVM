		...
		if(findUserSection(module, "name", userSectionIndex))
		{
			try
			{
				...
				while(stream.capacity())
				{
					...
					MemoryInputStream substream(
						stream.advance(numSubsectionBytes), numSubsectionBytes);
					...
					case NameSubsectionType::function:
					{
						...
						for(Uptr functionNameIndex = 0; functionNameIndex < numFunctionNames;
							++functionNameIndex)
						{
							U32 functionIndex = 0;
					...
					case NameSubsectionType::local:
					{
						...
						for(Uptr functionNameIndex = 0;
							functionNameIndex < numFunctionLocalNameMaps;
							++functionNameIndex)
						{
							U32 functionIndex = 0;
							serializeVarUInt32(substream, functionIndex);
							...
							{
								deserializeNameMap(
									substream, outNames.functions[functionIndex].locals);
							}
							else
							{
								Log::printf(
					...
					case NameSubsectionType::label:
					{
						...
						for(Uptr functionNameIndex = 0;
							functionNameIndex < numFunctionLabelNameMaps;
							++functionNameIndex)
						{
							U32 functionIndex = 0;
							serializeVarUInt32(substream, functionIndex);
							...
							{
								deserializeNameMap(
									substream, outNames.functions[functionIndex].labels);
							}
							else
							{
								Log::printf(
