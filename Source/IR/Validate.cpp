...
#define VALIDATE_FEATURE(context, feature) \
	...
	void validate(ValueType valueType)
	{
		...
		{
			throw ValidationException(
				"invalid value type (" + std::to_string((Uptr)valueType) + ")");
		}
	}
	...
	void validate(const Module& module, TableType type)
	{
		...
		if(type.isShared)
		{
			...
			VALIDATE_UNLESS(
				"shared tables must have a maximum size: ", type.size.max == UINT64_MAX);
		}
	}
	...
	void validate(const Module& module, MemoryType type)
	{
		...
		if(type.isShared)
		{
			...
			VALIDATE_UNLESS(
				"shared memories must have a maximum size: ", type.size.max == UINT64_MAX);
		}
	}
	...
	{
		if(importType.kind != expectedKind) { throw ValidationException("incorrect kind"); }
	}
	...
	FunctionType validateFunctionIndex(const Module& module, Uptr functionIndex)
	{
		VALIDATE_INDEX(functionIndex, module.functions.size());
		return module.types[module.functions.getType(functionIndex).index];
	}

	FunctionType validateBlockType(const Module& module, const IndexedBlockType& type)
	{
		switch(type.format)
		{
	...
	struct FunctionValidationContext
	{
		...
		void br_table(BranchTableImm imm)
		{
			...
			const TypeTuple defaultTargetParams
				= getBranchTargetByDepth(imm.defaultTargetDepth).params;
			...
			{
				const TypeTuple targetParams
					= getBranchTargetByDepth(targetDepths[targetIndex]).params;
				VALIDATE_UNLESS(
...
#undef VALIDATE_OP
	...
	private:
		...
		ValueType popAndValidateOperand(const char* context, const ValueType expectedType)
		{
			...
			{
				// If the current instruction is reachable, but the operand stack is empty, then
