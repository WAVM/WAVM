...
namespace Platform
{
	...
	bool describeInstructionPointer(Uptr ip, std::string& outDescription)
	{
		...
		U64 displacement;
		if(!dbgHelp->symFromAddr(GetCurrentProcess(), ip, &displacement, symbolInfo))
		{ return false; }
		else
		{
			outDescription = "host!";
		...
		void*& outExceptionData)
	{
		...
		{
			outExceptionData = reinterpret_cast<void*>(
				exceptionPointers->ExceptionRecord->ExceptionInformation[0]);
	...
	Thread* forkCurrentThread()
	{
		...
		else
		{
			...
			// Use the current stack pointer derive a conservative bounds on the area of this
			// thread's stack that is active.
			...
			{
				memcpy(
					forkedStackMaxAddr - numActiveStackBytes,
					minActiveStackAddr,
					numActiveStackBytes);
			}
	...
	Mutex::~Mutex() { DeleteCriticalSection((CRITICAL_SECTION*)&criticalSection); }

	void Mutex::lock() { EnterCriticalSection((CRITICAL_SECTION*)&criticalSection); }

	void Mutex::unlock() { LeaveCriticalSection((CRITICAL_SECTION*)&criticalSection); }
	...
	Event::~Event() { errorUnless(CloseHandle(handle)); }
	...
	void Event::signal() { errorUnless(SetEvent(handle)); }
	...
	bool closeFile(File* file) { return CloseHandle(filePointerToHandle(file)) != 0; }
	...
	bool flushFileWrites(File* file) { return FlushFileBuffers(filePointerToHandle(file)) != 0; }
