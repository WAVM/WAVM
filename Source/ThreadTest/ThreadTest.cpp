...
namespace ThreadTest
{
	...
	// Will always contain a null pointer in the first element so that 0 won't be allocated as a
	// thread ID.
	...
	// Adds the thread to the global thread array, assigning it an ID corresponding to its index in
	// the array.
	...
	// This function is just to provide a way to write to the currentThread thread-local variable in
	// a way that the compiler can't cache across a call to Platform::forkCurrentThread.
	FORCENOINLINE static void setCurrentThread(Thread* thread) { currentThread = thread; }

	// Validates that a thread ID is valid. i.e. 0 < threadId < threads.size(), and
	// threads[threadId] != null If the thread ID is invalid, throws an invalid argument exception.
	...
	static I64 threadEntry(Thread* thread)
	{
		...
		return invokeFunctionUnchecked(thread->context, thread->entryFunction, &thread->argument)
			->i64;
	}
		...
		I32 entryArgument)
	{
		...
		TableInstance* defaultTable
			= getTableFromRuntimeData(contextRuntimeData, defaultTableId.id);
		...
		// Increment the Thread's reference count for the pointer passed to the thread's entry
		// function. threadFunc calls the corresponding removeRef.
		thread->addRef();
	...
	DEFINE_INTRINSIC_FUNCTION_WITH_CONTEXT_SWITCH(threadTest, "forkThread", I64, forkThread)
	{
		...
		// Increment the Thread's reference count twice to account for the reference to the Thread
		// on the stack which is about to be forked. Each fork calls removeRef separately below.
		childThread->addRef(2);

		Platform::Thread* platformThread = Platform::forkCurrentThread();
		if(platformThread)
		{
			// Initialize the child thread's platform thread pointer, and allocate a thread ID for
			// it.
		...
		else
		{
			...
			// Since some compilers will cache a pointer to thread-local data that's accessed
			// multiple times in one function, and currentThread is accessed before calling
			// forkCurrentThread, we can't directly write to it in this function in case the
			// compiler tries to write to the original thread's currentThread variable. Instead,
			// call a FORCENOINLINE function (setCurrentThread) to set the variable.
	...
	// Validates a thread ID, removes the corresponding thread from the threads array, and returns
	// it.
