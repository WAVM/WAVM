		...
		Uptr sehTrampolineAddress)
	{
		...
		for(auto relocIt : section.relocations())
		{
			...
			if(relocIt.getType() == 3)
			{
				...
				{
					// Resolve __C_specific_handler to the trampoline previously created in the
					// image's 32-bit address space.
					const std::string symbolName = cantFail(symbol->getName()).str();
					if(symbolName == "__C_specific_handler")
					{ symbolAddress = sehTrampolineAddress; }
					else
					{
						symbolAddress = U64(cantFail(
							NullResolver::singleton->findSymbol(cantFail(symbol->getName()).str())
								.getAddress()));
					}
				}
				else
		...
		if(pdataCopy)
		{
			...
			if(PRINT_SEH_TABLES)
			{
				...
				for(Uptr functionIndex = 0; functionIndex < numFunctions; ++functionIndex)
				{
					...
					U8* unwindInfo                         = reinterpret_cast<U8*>(
                        imageBaseAddress + runtimeFunction.unwindInfoAddress);
					   ...
					   || (unwindInfoPrefix->flags & UnwindInfoFlags::UNW_FLAG_UHANDLER))
					{
						...
						for(Uptr entryIndex = 0; entryIndex < suffix->sehLSDA.numEntries;
							++entryIndex)
						{
							const SEHLanguageSpecificDataEntry& entry
								= suffix->sehLSDA.entries[entryIndex];
