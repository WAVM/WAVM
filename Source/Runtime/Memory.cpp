...
namespace Runtime
{
	...
	{
		// Iterate over all memories and check if the address is within the reserved address space
		// for each.
		Lock<Platform::Mutex> memoriesLock(memoriesMutex);
	...
	Iptr shrinkMemory(MemoryInstance* memory, Uptr numPagesToShrink)
	{
		...
		{
			// If the number of pages to shrink would cause the memory's size to drop below its
			// minimum, return -1.
