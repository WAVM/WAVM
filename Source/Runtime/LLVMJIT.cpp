...
namespace LLVMJIT
{
	...
	struct UnitMemoryManager : llvm::RTDyldMemoryManager
	{
			...
			U32 readWriteAlignment) override
		{
			...
			if(numAllocatedImagePages)
			{
				...
				readOnlySection.baseAddress = codeSection.baseAddress
	...
	// Encapsulates the LLVM JIT compilation pipeline but allows subclasses to define how the
	// resulting code is used.
	struct JITUnit
	{
	...
	private:
		...
		struct NotifyFinalizedFunctor
		{
			...
			void operator()(
				const llvm::orc::RTDyldObjectLinkingLayerBase::ObjHandleT& objectHandle);
		};
	...
	struct JITModule : JITUnit, JITModuleBase
	{
		...
		{
			// Save the address range this function was loaded at for future address->symbol
			// lookups.
...
#endif
			symbol
				= new JITSymbol(functionType, baseAddress, numBytes, std::move(offsetToOpIndexMap));
		}
	};
	...
	llvm::JITSymbol NullResolver::findSymbol(const std::string& name)
	{
		...
		Errors::fatalf(
			"LLVM generated code references disallowed external symbol: %s\n", name.c_str());
	}
	llvm::JITSymbol NullResolver::findSymbolInLogicalDylib(const std::string& name)
	{
		...
		const llvm::RuntimeDyld::LoadedObjectInfo& loadedObject)
	{
		...
		for(auto section : object->getBinary()->sections())
		{
			...
			{
				const U8* loadedSection
					= reinterpret_cast<U8*>(Uptr(loadedObject.getSectionLoadAddress(section)));
		...
		U8* trampolineBytes
			= jitUnit->memoryManager->allocateCodeSection(16, 16, 0, "seh_trampoline");
		trampolineBytes[0]         = 0xff;
		trampolineBytes[1]         = 0x25;
		*(U32*)&trampolineBytes[2] = 0;
		*(U64*)&trampolineBytes[6] = U64(
			cantFail(NullResolver::singleton->findSymbol("__C_specific_handler").getAddress()));
		...
		const llvm::orc::RTDyldObjectLinkingLayerBase::ObjHandleT& objectHandle)
	{
		for(Uptr objectIndex = 0; objectIndex < jitUnit->loadedObjects.size(); ++objectIndex)
		{
			...
			for(auto symbolSizePair : llvm::object::computeSymbolSizes(*object))
			{
				...
				// Expected<T> for each value unless it will be checked for success before
				// continuing.
				...
				llvm::DILineInfoTable lineInfoTable = dwarfContext->getLineInfoForAddressRange(
					loadedAddress, symbolSizePair.second);
				std::map<U32, U32> offsetToOpIndexMap;
			...
			if(jitUnit->pdataCopy)
			{
				...
				jitUnit->pdataCopy = nullptr;
			}
			if(jitUnit->xdataCopy)
			{
				delete[] jitUnit->xdataCopy;
				jitUnit->xdataCopy = nullptr;
			}
#endif
		}
	...
	void JITUnit::compile(const std::shared_ptr<llvm::Module>& llvmModule)
	{
		...
		// Verify the module.
		if(shouldLogMetrics && DUMP_UNOPTIMIZED_MODULE)
		{ printModule(llvmModule.get(), "llvmDump"); }
		...
		if(shouldLogMetrics && DUMP_OPTIMIZED_MODULE)
		{ printModule(llvmModule.get(), "llvmOptimizedDump"); }

		// Pass the module to the JIT compiler.
		Timing::Timer machineCodeTimer;
		handle = cantFail(compileLayer->addModule(llvmModule, NullResolver::singleton));
	...
	bool describeInstructionPointer(Uptr ip, std::string& outDescription)
	{
		...
		if(ip < symbol->baseAddress || ip >= symbol->baseAddress + symbol->numBytes)
		{ return false; }
		...
		CallingConvention callingConvention)
	{
		...
		// Load the function's arguments from an array of 64-bit values at an address provided by
		// the caller.
		...
		ValueVector results = emitContext.emitCallOrInvoke(
			functionPointer, arguments, functionType, callingConvention);
		...
		CallingConvention callingConvention)
	{
		...
		for(auto argIt = llvmFunction->args().begin() + 1; argIt != llvmFunction->args().end();
			++argIt)
		{ args.push_back(&*argIt); }

		llvm::Type* llvmNativeFunctionType
			= asLLVMType(functionType, callingConvention)->getPointerTo();
		llvm::Value* llvmNativeFunction
			= emitLiteralPointer(nativeFunction, llvmNativeFunctionType);
		ValueVector results = emitContext.emitCallOrInvoke(
			llvmNativeFunction, args, functionType, callingConvention);
...
#ifdef __APPLE__
		// Didn't figure out exactly why, but this works around a problem with the MacOS dynamic
		// loader. Without it, our symbols can't be found in the JITed object file.
		targetTriple += "-elf";
#endif
