...
namespace Runtime
{
	...
	TableInstance* createTable(Compartment* compartment, TableType type)
	{
		...
		// In 64-bit, allocate enough address-space to safely access 32-bit table indices without
		// bounds checking, or 16MB (4M elements) if the host is 32-bit.
	...
	TableInstance::~TableInstance()
	{
		...
		if(endOffset > 0)
		{
			Platform::freeVirtualPages(
				(U8*)baseAddress, (endOffset >> pageBytesLog2) + numGuardPages);
		}
		baseAddress = nullptr;
	...
	Object* setTableElement(TableInstance* table, Uptr index, Object* newValue)
	{
		...
		// Use a saturated index to access the table data to ensure that it's harmless for the CPU
		// to speculate past the above bounds check.
		const Uptr saturatedIndex = Platform::saturateToBounds(index, table->elements.size());

		// Write the new table element to both the table's elements array and its indirect function
		// call data.
	...
	Uptr getTableNumElements(TableInstance* table) { return table->elements.size(); }
