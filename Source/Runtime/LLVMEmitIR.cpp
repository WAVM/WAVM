...
namespace LLVMJIT
{
	...
	struct EmitModuleContext
	{
		...
		, diBuilder(*llvmModule)
		{
			...
			diValueTypes[(uintp)ValueType::v128] = diBuilder.createBasicType("v128",128,llvm::dwarf::DW_ATE_signed);
			diValueTypes[(uintp)ValueType::b8x16] = diBuilder.createBasicType("b8x16",128,llvm::dwarf::DW_ATE_boolean);
			diValueTypes[(uintp)ValueType::b16x8] = diBuilder.createBasicType("b16x8",128,llvm::dwarf::DW_ATE_boolean);
			diValueTypes[(uintp)ValueType::b32x4] = diBuilder.createBasicType("b32x4",128,llvm::dwarf::DW_ATE_boolean);
			diValueTypes[(uintp)ValueType::b64x2] = diBuilder.createBasicType("b64x2",128,llvm::dwarf::DW_ATE_boolean);
			#endif
	...
	struct EmitFunctionContext
	{
		...
		{
			const uintp numLanes = boolVector->getType()->getVectorNumElements();
		...
		void launch_thread(LaunchThreadImm)
		{
			...
			auto defaultTableAsI64 = emitLiteral(reinterpret_cast<uint64>(moduleContext.moduleInstance->defaultTable));
		...
		void trapIfMisalignedAtomic(llvm::Value* address,uint32 naturalAlignmentLog2)
		{
			if(naturalAlignmentLog2 > 0)
			{
				emitConditionalTrapIntrinsic(
					irBuilder.CreateICmpNE(
						typedZeroConstants[(uintp)ValueType::i32],
						irBuilder.CreateAnd(address,emitLiteral((uint32(1) << naturalAlignmentLog2) - 1))),
	...
	llvm::Module* EmitModuleContext::emit()
	{
		...
		Timing::logRatePerSecond("Emitted LLVM IR",emitTimer,(float64)llvmModule->size(),"functions");
