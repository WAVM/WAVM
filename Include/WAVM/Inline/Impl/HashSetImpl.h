...
#define WAVM_INLINE_IMPL_HASHSETIMPL_H
...
namespace WAVM {
	...
	{
		WAVM_ASSERT(bucket && bucket->hashAndOccupancy);
	...
	template<typename Element, typename ElementHashPolicy>
	HashSet<Element, ElementHashPolicy>::HashSet(
		const std::initializer_list<Element>& initializerList)
	...
	const Element& HashSet<Element, ElementHashPolicy>::operator[](const Element& element) const
	{
		...
		if(!bucket)
		{
			// In addition to the assert, use WAVM_UNREACHABLE to ensure that GCC's
			// -Wnull-dereference warning isn't triggered because it thinks this function might
			// return nullptr.
			WAVM_UNREACHABLE();
		}
	...
	bool HashSet<Element, ElementHashPolicy>::contains(const Element& element) const
	{
		...
		const HashTableBucket<Element>* bucket = table.getBucketForRead(hash, element);
		WAVM_ASSERT(!bucket
					|| bucket->hashAndOccupancy
						   == (hash | HashTableBucket<Element>::isOccupiedMask));
	...
	HashSetIterator<Element> HashSet<Element, ElementHashPolicy>::begin() const
	{
		...
		if(!beginBucket) { return HashSetIterator<Element>(nullptr, nullptr); }
...
#endif // WAVM_INLINE_IMPL_HASHSETIMPL_H
