...
#define WAVM_INLINE_IMPL_HASHTABLEIMPL_H
...
#define HASHTABLE_ARGUMENTS Key, Element, HashTablePolicy, AllocPolicy
		...
		const LookupKey& key) const
	{
		...
		while(true)
		{
			...
			else
			{
				...
				{
					// If the bucket holds a key whose ideal bucket follows the specified key's
					// ideal bucket, then the following buckets cannot hold the key, so return null.
					return nullptr;
				}
				else
				{
	...
	template<HASHTABLE_PARAMETERS>
	HashTableBucket<Element>& HashTable<HASHTABLE_ARGUMENTS>::getBucketForAdd(Uptr hash,
																			  const Key& key)
	{
		...
		for(Uptr idealBucketIndex = 0; idealBucketIndex < numBuckets(); ++idealBucketIndex)
		{
			...
			while(true)
			{
				...
				if(!buckets[bucketIndex].hashAndOccupancy
				   || probeCount > calcProbeCount(bucketIndex))
				{
					break;
				}
				++probeCount;
	...
	template<HASHTABLE_PARAMETERS>
	HashTable<HASHTABLE_ARGUMENTS>::HashTable(HashTable&& movee) noexcept
	{
	...
	void HashTable<HASHTABLE_ARGUMENTS>::moveFrom(HashTable&& movee) noexcept
	{
		...
		else
		{
			...
			movee.numElements = 0;
			movee.hashToBucketIndexMask = UINTPTR_MAX;
...
#endif // WAVM_INLINE_IMPL_HASHTABLEIMPL_H
