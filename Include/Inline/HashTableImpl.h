	...
	while(true)
	{
		...
		if(!bucket.hashAndOccupancy)
		{
			...
			&& HashTablePolicy::areKeysEqual(
				HashTablePolicy::getKey(buckets[bucketIndex].storage.contents),
				key
				))
		{
...
template<HASHTABLE_PARAMETERS>
HashTableBucket<Element>& HashTable<HASHTABLE_ARGUMENTS>::getBucketForAdd(
	Uptr hash,
	const Key& key
	)
{
	...
	)
{
	...
	while(true)
	{
		...
		if(!bucket.hashAndOccupancy)
		{
			...
			&& HashTablePolicy::areKeysEqual(
				HashTablePolicy::getKey(bucket.storage.contents),
				key
				))
		{
...
template<HASHTABLE_PARAMETERS>
HashTable<HASHTABLE_ARGUMENTS>::~HashTable()
{
	destruct();
}
